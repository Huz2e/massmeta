/obj/item/compressionkit
	...
	icon = 'modular_meta/features/compression_kit/icons/comp_kit.dmi'
	icon_state = "compkit"
...
/obj/item/compressionkit/suicide_act(mob/living/carbon/M) // Работает лишь на половину, осталось после переноса с beestation
	...
	var/list/organs = M.get_bodypart(BODY_ZONE_HEAD) + M.get_bodypart(BODY_ZONE_PRECISE_EYES) + M.get_bodypart(BODY_ZONE_PRECISE_MOUTH)
	...
	M.add_splatter_floor(T, 10)
	playsound(M, 'sound/items/weapons/flash.ogg', 50, 1)
	playsound(M, 'sound/effects/splat.ogg', 50, 1)

	return OXYLOSS

/obj/item/compressionkit/interact_with_atom(atom/target, mob/living/user, list/modifiers)
	. = ..()
	if(charges == 0)
